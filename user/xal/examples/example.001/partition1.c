...
void ExecTimerHandler(trapCtxt_t *ctxt)                                     /* XAL trap API */
{
    ...
    PRINT("[%ld:%ld] IRQ EXEC Timer\n", (xm_u32_t) hw, (xm_u32_t) exec);
    ++lock;
}
...
//#ifndef CONFIG_ARM
#ifdef CONFIG_ARM
    InstallIrqHandler(XAL_XMEXT_TRAP(XM_VT_EXT_EXEC_TIMER), ExecTimerHandler);         /* Install timer handler */
    HwSti();                                                                /* Enable irqs */
#elif CONFIG_LEON3FT    
    ...
    XM_set_timer(XM_EXEC_CLOCK, execClock+5000LL, 1000000LL);       /* Set hardware time driven timer */
